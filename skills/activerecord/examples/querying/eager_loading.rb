...
Author.preload(:posts)  # 2 queries
...
# includes (better)
Post.includes(:author, :comments).each { |p|
  p.author
  p.comments.to_a
}
...
Post.eager_load(:author, :comments).each { |p|
  p.author
  p.comments.to_a
}
...
Post.preload(:author, :comments).each { |p|
  p.author
  p.comments.to_a
}
