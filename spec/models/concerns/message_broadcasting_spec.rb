...
RSpec.describe Message::Broadcasting do
  ...
  describe "after_create_commit" do
    it "broadcasts the message to the session stream" do
      expect {
        create_message
    ...
    it "includes the message database ID in the broadcast" do
      event = nil
      expect {
    ...
    it "broadcasts tool_call messages" do
      expect {
        session.messages.create!(
  ...
  describe "viewport eviction" do
    it "includes evicted_message_ids when old events leave the viewport" do
      # Fill the viewport with two large events
      old = create_message(token_count: 100_000, timestamp: 1)
      session.snapshot_viewport!([old.id])

      # Create another large event — old event should be evicted
      expect {
        create_message(token_count: 100_000, timestamp: 2)
      }.to have_broadcasted_to("session_#{session.id}")
        .with(a_hash_including("evicted_message_ids" => [old.id]))
    end

    it "omits evicted_message_ids when no events are evicted" do
      session.snapshot_viewport!([])

      expect {
        create_message(token_count: 10, timestamp: 1)
      }.to have_broadcasted_to("session_#{session.id}")
        .with(satisfy { |payload| !payload.key?("evicted_message_ids") })
    end

    it "updates the viewport snapshot after broadcasting" do
      event = nil
      expect {
        event = create_message(token_count: 10, timestamp: 1)
      }.to have_broadcasted_to("session_#{session.id}")

      expect(session.reload.viewport_message_ids).to include(event.id)
    end
  end

  describe "after_update_commit" do
    it "broadcasts with action update when message is updated" do
      event = create_message
