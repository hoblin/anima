...
RSpec.describe Tools::Remember do
  ...
  def create_event(sess, type:, content: "msg", tool_name: nil)
  ...
  describe "#execute" do
    ...
    it "includes events from before and after the target" do
      create_event(session, type: "user_message", content: "Before target")
    ...
    it "compresses tool_response events to status indicators" do
      create_event(session, type: "user_message", content: "Do something")
      tc = create_event(session, type: "tool_call", tool_name: "bash")
    ...
    it "includes snapshots before the center range when available" do
      ...
      5.times { create_event(session, type: "user_message", content: "Early message") }
      early_ids = session.events.pluck(:id)

      session.snapshots.create!(
        text: "Earlier discussion about deployment.",
        from_event_id: early_ids.first, to_event_id: early_ids.last,
