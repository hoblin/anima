...
RSpec.describe Mneme::PassiveRecall do
  ...
  describe "#call" do
    ...
    it "finds events matching active goal descriptions" do
      ...
      expect(results.map(&:event_id)).to include(event.id)
    end

    it "excludes events already in the current viewport" do
      # Event in current session's viewport
      event = create_event(session, type: "user_message",
      ...
      expect(results.map(&:event_id)).not_to include(event.id)
    end
    ...
    it "ignores completed sub-goals" do
      other_session = Session.create!
      create_event(other_session, type: "user_message",
