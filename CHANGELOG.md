...
- **FTS5 schema dump fix** — initializer patches Rails' SQLite3 schema dumper to skip FTS5 virtual tables (contentless tables crash the Ruby DSL dumper); FTS5 lives in migrations only (#252)
...
### Fixed
- **Bash tool "Shell session is not running" after timeout** — replaced unsafe `Timeout.timeout` (which uses `Thread.raise` that corrupts mutex state and causes exceptions to fire outside handler blocks when nested) with `IO.select`-based deadline tracking for all PTY reads; shell sessions now auto-respawn after death (timeout, crash, `exit`) preserving working directory, modeled after `MCP::StdioTransport`; timeout errors now include partial stdout/stderr so the agent can see why a command hung (#261)
- **Git commands hang in bash tool** — `git log` and other git commands could hang indefinitely because the PTY made git launch a pager (`less`) waiting for interactive input; shell now sets `GIT_PAGER=cat`, `PAGER=cat`, and `GIT_TERMINAL_PROMPT=0` to prevent pager and credential prompt hangs (#261)
- **Shell teardown resource leak** — `shutdown` (now `teardown`) guarded with `return unless @alive`, so when a failed timeout recovery already set `@alive = false`, the PTY process, FIFO, and stderr thread were never cleaned up; teardown now always runs regardless of `@alive` state (#261)
...
### Added
- **CI smoke test workflow** — builds and installs the gem in a clean environment, verifies Gemfile is excluded and Rails boots without Bundler; runs on PRs, pushes to main, and blocks releases (#225)
